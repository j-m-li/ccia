...
%.i386_s2.o: %.c $(TARGET_I386)
	./$(TARGET_I386) -Iinclude -DTARGET_I386 -c -o $@ $<
...
%.i386_s3.o: %.c $(STAGE2_I386_TARGET)
	./$(STAGE2_I386_TARGET) -Iinclude -DTARGET_I386 -c -o $@ $<
...
self-i386: $(STAGE2_I386_TARGET) $(STAGE3_I386_TARGET)
	...
	@for f in $(SRCS_I386); do \
		./$(STAGE2_I386_TARGET) -Iinclude -DTARGET_I386 -S -o $${f%.c}.i386_s2.s $$f || exit 1; \
		./$(STAGE3_I386_TARGET) -Iinclude -DTARGET_I386 -S -o $${f%.c}.i386_s3.s $$f || exit 1; \
...
test: $(TARGET_X86_64) $(TARGET_I386)
	...
	@CC90=./$(TARGET_X86_64) bash tests/run_tests.sh
	@echo "=== Running test suite with cc90-i386 (x86 32-bit) ==="
	@CC90=./$(TARGET_I386) bash tests/run_tests.sh
...
test-self: self self-i386
	...
	@CC90=./$(TARGET_I386) bash tests/run_tests.sh
	@CC90=./$(STAGE2_I386_TARGET) bash tests/run_tests.sh
	@CC90=./$(STAGE3_I386_TARGET) bash tests/run_tests.sh
